...
void HttpServer::start()
{
    ...
    {
        HTTP_SERVER(INFO, m_sslCertInfo->moduleNameForLog())
            << LOG_BADGE("startListen") << LOG_DESC("http server is running");
        return;
    }

    HTTP_SERVER(INFO, m_sslCertInfo->moduleNameForLog())
        << LOG_BADGE("startListen") << LOG_KV("listenIP", m_listenIP)
    ...
    if (ec)
    {
        HTTP_SERVER(WARNING, m_sslCertInfo->moduleNameForLog())
            << LOG_BADGE("open") << LOG_KV("error", ec) << LOG_KV("message", ec.message());
    ...
    {
        HTTP_SERVER(WARNING, m_sslCertInfo->moduleNameForLog())
            << LOG_BADGE("set_option") << LOG_KV("error", ec) << LOG_KV("message", ec.message());
    ...
    {
        HTTP_SERVER(WARNING, m_sslCertInfo->moduleNameForLog())
            << LOG_BADGE("bind") << LOG_KV("error", ec) << LOG_KV("message", ec.message());
    ...
    {
        HTTP_SERVER(WARNING, m_sslCertInfo->moduleNameForLog())
            << LOG_BADGE("listen") << LOG_KV("error", ec) << LOG_KV("message", ec.message());
    ...
    HTTP_SERVER(INFO, m_sslCertInfo->moduleNameForLog())
        << LOG_BADGE("startListen") << LOG_KV("ip", endpoint.address().to_string())
...
void HttpServer::stop()
{
    ...
    HTTP_SERVER(INFO, m_sslCertInfo->moduleNameForLog())
        << LOG_BADGE("stop") << LOG_DESC("http server");
}
...
void HttpServer::onAccept(boost::beast::error_code ec, boost::asio::ip::tcp::socket socket)
{
    ...
    {
        HTTP_SERVER(WARNING, m_sslCertInfo->moduleNameForLog())
            << LOG_BADGE("accept") << LOG_KV("error", ec) << LOG_KV("message", ec.message());
        return doAccept();
    }
    ...
    HTTP_SERVER(INFO, m_sslCertInfo->moduleNameForLog())
        << LOG_BADGE("accept") << LOG_KV("local_endpoint", socket.local_endpoint())
    ...
    {  // non ssl , start http session
        auto httpStream = m_httpStreamFactory->buildHttpStream(
            ...
            m_sslCertInfo->moduleNameForLog());
    ...
    ss->async_handshake(boost::asio::ssl::stream_base::server,
        [this, ss, localEndpoint, remoteEndpoint, endpointPublicKey, self](
            boost::beast::error_code _ec) {
            if (_ec)
            {
                HTTP_SERVER(INFO, m_sslCertInfo->moduleNameForLog())
                    << LOG_BADGE("async_handshake") << LOG_DESC("ssl handshake failed")
                    << LOG_KV("local", localEndpoint) << LOG_KV("remote", remoteEndpoint)
            ...
            if (server)
            {
                auto httpStream = server->httpStreamFactory()->buildHttpStream(
                    ss, m_sslCertInfo->moduleNameForLog());
...
{
    auto session = std::make_shared<HttpSession>(_httpStream->moduleNameForLog());
    ...
    queue->setSender([this, self](HttpResponsePtr _httpResp) {
        ...
        HTTP_SESSION(TRACE, m_sslCertInfo->moduleNameForLog())
            << LOG_BADGE("Queue::Write") << LOG_KV("resp", _httpResp->body())
    ...
    std::shared_ptr<boost::asio::ssl::context> _ctx, std::string _moduleNameForLog)
{
    ...
    HTTP_SERVER(INFO, _moduleNameForLog)
        << LOG_BADGE("buildHttpServer") << LOG_KV("listenIP", _listenIP)
