...
void WsService::status()
{
    auto ss = sessions();
    WEBSOCKET_STAT(INFO) << LOG_DESC("connected nodes") << LOG_KV("count", ss.size());
    for (const auto& session : ss)
    {
        ...
        {
            WEBSOCKET_STAT(INFO) << LOG_BADGE("stat") << LOG_KV("endpoint", session->endPoint())
                                 << LOG_KV("writeQueueSize", writeQueueSize)
                                 << LOG_KV("callbackQueueSize", callbackQueueSize);
        }
        else
        {
            WEBSOCKET_STAT(DEBUG) << LOG_BADGE("stat") << LOG_KV("endpoint", session->endPoint())
                                  << LOG_KV("writeQueueSize", writeQueueSize)
                                  << LOG_KV("callbackQueueSize", callbackQueueSize);
        }
    }
