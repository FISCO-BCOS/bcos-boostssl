...
{
    auto genError = [](std::string error, std::string host, uint16_t port, bool end) {
    ...
    for (std::size_t i = 0; i < vPromise->size(); ++i)
    {
        ...
        case std::future_status::timeout:
            errorMsg += genError("connection timeout", (*_peers)[i].host, (*_peers)[i].port,
                i == vPromise->size() - 1);
            break;
        case std::future_status::ready:
            ...
            try
            {
                ...
                if (ec)
                {
                    errorMsg += genError(ec.message(), (*_peers)[i].host, (*_peers)[i].port,
                        i == vPromise->size() - 1);
                }
                else
                {
    ...
    if (sucCount == 0)
    {
        ...
        BOOST_THROW_EXCEPTION(std::runtime_error(
            "Failed to connect to server: [" + boost::to_lower_copy(errorMsg) + "]"));
        return;
    }
}
