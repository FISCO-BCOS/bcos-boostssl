...
{
    std::cerr << " boostssl_delay_perf work in [client] model. \n"
              << " \t serverIp: " << serverIp << "\n"
    ...
    while (i++ < echoC)
    {
        ...
        wsService->asyncSendMessage(msg, Options(-1),
            [&p, &nFailedC, &nSucC](Error::Ptr _error, std::shared_ptr<WsMessage> _msg,
...
void serverWork(std::string listenIp, uint16_t listenPort, bool disableSsl)
{
    std::cerr << " boostssl_delay_perf work in [server] model." << std::endl
    ...
    while (true)
    {
        BCOS_LOG(INFO) << LOG_BADGE(" boostssl deplay perf server working ...");
...
int main(int argc, char** argv)
{
    ...
    if (workModel == "server")
    {
        serverWork(host, port, disableSsl);
    }
    else if (workModel == "client")
    {
        ...
        clientWork(host, port, disableSsl, echoCount, msgSize);
    }
    else
    {
        usage();
